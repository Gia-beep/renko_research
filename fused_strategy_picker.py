...
def pick_stocks(tq, stock_list):
    """
    第二层 & 第三层：选股与买点触发器
    基于：量价VWAP共振 + 砖形图强红结构 + 双均线 stack（黄线之下不操作）
    """
    print(f">>> 开始获取 {len(stock_list)} 只股票的K线数据...")

    # 获取过去 70 个交易日的日线数据（用于计算 20 日 / 60 日均线）
    ...
    # --- 指标计算 ---
    # 1. 均线系统 — renko.md §3.2 双均线 stack: 白线(20) / 黄线(60)
    ma_white_close = close_df.rolling(window=20).mean()
    ma_yellow_close = close_df.rolling(window=60).mean()
    # 2. 成交量均线
    ...
    # 4. 双均线 stack（renko.md §3.2「黄线之下不操作」）：
    #    收盘价站上黄线 AND 白线在黄线之上（多头排列）
    is_dual_ma_bull = (latest_close > latest_ma_yellow) & (latest_ma_white > latest_ma_yellow)
    ...
    if selected_stocks:
        ...
        print(f"{'代码':<12} {'收盘价':<8} {'VWAP':<8} {'MA20':<8} {'MA60':<8} {'放量倍数':<10} {'实体比例'}")
        print("-" * 75)
        for code in selected_stocks:
            vol_ratio = latest_vol[code] / latest_ma20_vol[code]
            body_ratio = body[code] / total_range[code]
            print(f"{code:<12} {latest_close[code]:<8.2f} {latest_vwap[code]:<8.2f} "
                  f"{latest_ma_white[code]:<8.2f} {latest_ma_yellow[code]:<8.2f} "
                  f"{vol_ratio:<10.2f} {body_ratio:.1%}")
    else:
...
def check_exit_signals(tq, entry_records):
    """For each currently-held name, evaluate the renko.md §5.1/§5.2 sell rules.

    Parameters
    ----------
    tq : Tongdaxin API handle (the same object returned by ``init_tdx``).
    entry_records : dict
        ``{code: {"entry_date": "YYYY-MM-DD" or pd.Timestamp,
                  "entry_close": float, "red_brick_count": int}}``.
        ``red_brick_count`` is the cumulative number of red bricks observed
        since entry (entry bar itself counts as #1 per §5.2).  If omitted,
        defaults to 0 ⇒ the four-brick exit will not fire on the live bar
        unless you tally bricks externally.

    Returns
    -------
    list[str] : codes that should be sold today, plus a printed reason table.
    """
    if not entry_records:
        return []

    held = list(entry_records.keys())
    print(f"\n>>> 检查 {len(held)} 只持仓的出场信号...")

    # Need at least 60 + 1 bars for MA60; pull ~80 for safety.
    data = tq.get_market_data(
        field_list=['Open', 'High', 'Low', 'Close', 'Volume', 'Amount'],
        stock_list=held,
        period='1d',
    ...
    if not data or 'Close' not in data:
        print(">>> 持仓出场检查：未能获取K线数据。")
        return []

    from strategy_core import compute_exit_conditions

    decisions = compute_exit_conditions(data, entry_records, red_brick=None)
    to_sell = decisions.index[decisions["should_exit"]].tolist()

    print(f"{'代码':<12} {'卖出':<6} {'原因':<24} {'收盘':<8} {'白线连破':<8} {'砖数':<6}")
    print("-" * 72)
    for code, row in decisions.iterrows():
        flag = "卖" if row["should_exit"] else "持"
        print(
            f"{code:<12} {flag:<6} {row['exit_reason']:<24} "
            f"{row['close']:<8.2f} {row['white_break_streak']:<8d} "
            f"{row['red_brick_count']:<6d}"
        )

    if to_sell:
        print(f"\n>>> 建议今日卖出 {len(to_sell)} 只：{to_sell}")
    else:
        print("\n>>> 持仓无触发出场条件，继续持有。")
    return to_sell
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 3. 持仓出场检查（如果 results/holdings.json 存在）
        holdings_path = os.path.join(
            os.path.dirname(__file__), "results", "holdings.json"
        )
        if os.path.exists(holdings_path):
            import json
            try:
                with open(holdings_path, "r", encoding="utf-8") as fh:
                    entry_records = json.load(fh)
                check_exit_signals(tq, entry_records)
            except Exception as e:
                print(f">>> 持仓出场检查失败 ({e})，跳过。")
        else:
            print(f">>> 未找到 {holdings_path}，跳过持仓出场检查。")

        # 4. 准备股票池（这里以几个测试股票或某个板块成分股为例）
